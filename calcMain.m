...
% @author      Rafael Anderka, Lorenzo Principe
    ...
    switch state.mode
        case 1 % Acceleration
            ...
            state.fx(i)           = calcFx(state.frequency(i), state.velocity(i - 1), parameters);
        ...
        case 2 % Deceleration using EmBrakes            
            state.frequency(i)    = 0;
            state.fx(i)           = calcBrakingForce(state.velocity(i-1), parameters);  
    ...
    state.fx(i) = state.fx(i) - state.drag(i) - state.rollFriction(i);
    ...
    efficiency(i) = state.power(i) / state.powerInput(i);
